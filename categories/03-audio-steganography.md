...
- [WaveVerify (Audio Watermarking Beating AudioSeal/WavMark)](#waveverify-audio-watermarking-beating-audiosealwavmark)
- [WAKE (Watermarking Audio with Key Enrichment)](#wake-watermarking-audio-with-key-enrichment)
- [Separation-First Multi-Stream Audio Watermarking](#separation-first-multi-stream-audio-watermarking)
- [Watermarking Training Data of Music Generation Models](#watermarking-training-data-of-music-generation-models)
...
### WaveVerify (Audio Watermarking Beating AudioSeal/WavMark)

**Goal:** Audio watermarking framework for media authentication and deepfake detection that achieves zero Bit Error Rate under common distortions and MIoU ≥ 0.98 under severe temporal modifications, outperforming AudioSeal and WavMark.

| Algorithm | Year | Architecture | Notable Feature |
|-----------|------|--------------|-----------------|
| **WaveVerify** | 2025 | Multi-scale neural watermark | Zero BER under common distortions; MIoU ≥ 0.98 under temporal cropping/stretching [[1]](https://arxiv.org/abs/2507.21150) |

**State of the art:** Surpasses AudioSeal and WavMark on standard benchmarks. Localized robustness via multi-scale watermark structure; survives heavy editing including time-stretch and partial removal.

**Production readiness:** Research
Reference implementation in paper; not yet deployed in commercial pipelines.

**Security status:** Robust
Robust against common distortions (codec, noise, EQ) and adversarial temporal editing. Pitch-shift attacks not yet fully addressed in initial benchmark.

**Community acceptance:** Emerging
Recent (mid-2025); positioning as next-generation audio watermark for deepfake authentication.

---

### WAKE (Watermarking Audio with Key Enrichment)

**Goal:** Improve perceptual quality (SNR, PESQ) and decoding accuracy of audio watermarks via key-enriched embedding that outperforms AudioSeal and WavMark on both fidelity and bit-recovery axes.

| Algorithm | Year | Architecture | Notable Feature |
|-----------|------|--------------|-----------------|
| **WAKE** | 2025 | Key-enriched neural watermark | Higher SNR/PESQ than AudioSeal/WavMark; improved decoding accuracy [[1]](https://arxiv.org/abs/2506.05891) |

**State of the art:** Beats AudioSeal and WavMark on both perceptual quality (SNR/PESQ) and decoding accuracy. Key enrichment defeats single-watermark-template overwriting attacks.

**Production readiness:** Research
Academic prototype; no production deployment yet.

**Security status:** Robust
Defeats simple template-overwriting attacks via key enrichment. Robustness under generative neural codec compression not exhaustively tested.

**Community acceptance:** Emerging
Recent (mid-2025); cited as candidate replacement for AudioSeal in robustness-critical applications.

---

### Separation-First Multi-Stream Audio Watermarking

**Goal:** Watermark audio in the "separation-first" paradigm where the watermark survives source separation (e.g., music vocals/drums/bass extraction) — relevant for music streaming services where separated stems may be redistributed.

| Algorithm | Year | Architecture | Notable Feature |
|-----------|------|--------------|-----------------|
| **Separation-First Multi-Stream** | 2026 | Joint-trained encoder-separator | Watermark survives source separation in music generation contexts [[1]](https://arxiv.org/abs/2603.16805) |

**State of the art:** First feasible joint training for separation-resilient watermarking. Targets generative music model output authentication where stems may be separated and re-distributed.

**Production readiness:** Research
Academic prototype.

**Security status:** Robust (within separation use case)
Designed against music-source-separation pipelines (Demucs, OpenUnmix); not yet tested against neural audio codecs.

**Community acceptance:** Emerging
Recent (early 2026); fills gap in generative-music watermarking robustness.

---

### Watermarking Training Data of Music Generation Models

**Goal:** Embed watermarks into the training data of generative music models so that any music generated by a trained model carries an attributable signature, even if the model itself is opaque or distilled.

| Algorithm | Year | Architecture | Notable Feature |
|-----------|------|--------------|-----------------|
| **Music Training Data Watermark** | 2024 | Pre-training data watermark | Survives training; allows attribution from model output [[1]](https://arxiv.org/abs/2412.08549) |

**State of the art:** Differs from post-hoc audio watermarking — watermarks embedded at the training-data stage propagate through the learned model to its outputs, enabling provenance tracking for AI-generated music.

**Production readiness:** Research
Academic prototype; demonstrated on music generation models.

**Security status:** Secure (against post-hoc detection bypass)
Watermark is learned, not added; cannot be removed by simple post-processing.

**Community acceptance:** Emerging
Late-2024 paper; influences debate over copyright attribution for AI-generated music.
