# Physical & Social Steganography

<!-- TOC -->
## Contents (2 subcategories)

**[Physical Methods](#physical-methods)**
- [Morse Code Yarn](#morse-code-yarn)
- [Music Cipher](#music-cipher)
- [Printer Steganography](#printer-steganography)
- [Microdots](#microdots)
- [Invisible Ink](#invisible-ink)

**[Social Steganography](#social-steganography)**
- [Cultural References](#cultural-references)
- [Contextual Hiding](#contextual-hiding)
<!-- /TOC -->

## Physical Methods

---

### Morse Code Yarn

**Goal:** Hide messages in knitted clothing using Morse code.

| Algorithm | Year | Principle | Note |
|-----------|------|-----------|------|
| **Morse Yarn** | 1940s | Morse on yarn | WWII spy technique |

**State of the art:** Historical method.

**Production readiness:** Deprecated

**Security status:** N/A — Historical

**Community acceptance:** Niche — Historical curiosity

---

### Music Cipher

**Goal:** Hide messages in musical notes.

| Algorithm | Year | Principle | Note |
|-----------|------|-----------|------|
| **Music Cipher** | 1985 | Note pitch encoding | Klezmer saxophonist |

**State of the art:** Unique approach using music.

**Production readiness:** Deprecated

**Security status:** Caution

**Community acceptance:** Niche

---

### Printer Steganography

**Goal:** Track documents via hidden dots.

| Algorithm | Year | Principle | Note |
|-----------|------|-----------|------|
| **Printer Dots** | 2005 | Yellow dot matrix | Serial + timestamp |

**State of the art:** Standard in color laser printers.

**Production readiness:** Production

**Implementations:**
- [Printer Steganography Detector](https://github.com/abe-modyo/printer-steganography) ⭐ 89

**Security status:** Broken — Documented tracking

**Community acceptance:** Standard — Well-documented

---

### Microdots

**Goal:** Hide microscopic images in documents.

| Algorithm | Year | Principle | Note |
|-----------|------|-----------|------|
| **Microdots** | 2000 | Subroscopic images | Historical |

**State of the art:** Historical spy technique.

**Production readiness:** Deprecated

**Security status:** Caution

**Community acceptance:** Niche

---

### Invisible Ink

**Goal:** Hidden writing revealed by specific conditions.

| Algorithm | Year | Principle | Note |
|-----------|------|-----------|------|
| **Invisible Ink** | 1800s | Chemical reveal | Historical |

**State of the art:** Historical method.

**Production readiness:** Deprecated

**Security status:** Caution

**Community acceptance:** Niche

---

## Social Steganography

---

### Cultural References

**Goal:** Hide meaning in cultural symbols.

| Algorithm | Year | Principle | Note |
|-----------|------|-----------|------|
| **Cultural References** | 2013 | Pop culture symbols | Context-dependent |

**State of the art:** Active in censored communities.

**Production readiness:** Production

**Security status:** Secure — No technical detection

**Community acceptance:** Standard — Active in censored communities

---

### Contextual Hiding

**Goal:** Hide messages in contextual meaning.

| Algorithm | Year | Principle | Note |
|-----------|------|-----------|------|
| **Contextual Hiding** | 2013 | Idioms, memes | Visible to initiates |

**State of the art:** Used in protest movements.

**Production readiness:** Production

**Security status:** Secure — No technical detection

**Community acceptance:** Standard — Active in censored communities
